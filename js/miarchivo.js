...
const verCard = (n) => {
  ...
  for (const propiedad in selectedSVC) {
    if (propiedad != `Nombre`) {
      ...
      if (propiedad == "id") idaux = selectedSVC[propiedad];
      datos.append(prop);
    }
  }
...
const agregarACarrito = (id) => {
  ...
  fetch("../js/productos.json")
    .then((res) => {
      if (!res.ok) {
        console.error("Error cargando los productos");
    ...
    .then((data) => {
      ...
      // Validar que el producto existe en el archivo JSON
      if (!itemElegido) {
        console.error("El producto no existe en el archivo JSON");
        return;
      }
